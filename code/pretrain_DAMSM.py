...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        ...
                        default='cfg/DAMSM/fashion_gen.yml', type=str)
...
def evaluate(dataloader, cnn_model, rnn_model, batch_size):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        words_features, sent_code = cnn_model(real_imgs[-1]) ##### word_features: image region features, Sent_code: GAP code (Single)
        # nef = words_features.size(1)
        # words_features = words_features.view(batch_size, nef, -1)

        hidden = rnn_model.init_hidden(batch_size)
        words_emb, sent_emb = rnn_model(captions, cap_lens, hidden) ##### words_emb: word! Sent_code: sent!
